...
int main(int argc, char* argv[]) {
    // CEF subprocess check -- must happen before anything else
    ...
    // Load file if in player mode
    if (player_mode) {
        const char* loadcmd[] = {"loadfile", player_playlist[0].c_str(), NULL};
        mpv_command(mpv, loadcmd);
    }

    // --- Wait for VO (mpv needs a window before we can get Wayland handles) ---
    ...
    while (true) {
        ...
        // force-window=yes means video-params will fire even without a file
        // Check osd-dimensions as fallback
        int64_t ow = 0;
    ...
    if (xdg_cache && xdg_cache[0]) {
        ...
        cache_path = std::string(home ? home : "/tmp") + "/.cache/jellyfin-desktop";
    }
    ...
    // multi_threaded_message_loop: CEF loads on its own thread
    g_client->waitForLoad();
