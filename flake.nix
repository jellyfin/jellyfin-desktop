{
  ...
  inputs = {
    ...
    # (mpv-player/mpv#9654 is the open upstream feature request).
    #
    # Pinned to match the third_party/mpv git submodule. Bump to whatever
    # `git -C third_party/mpv rev-parse HEAD` prints if upstream advances.
    mpv-cef-src = {
  ...
  outputs =
    ...
    flake-utils.lib.eachSystem [ "x86_64-linux" ] (
      ...
      let
        ...
        # ----------------------------------------------------------------------
        # Version metadata (read from the same files the CMake build reads)
        # ----------------------------------------------------------------------
        appVersion = lib.fileContents ./VERSION;

        # CEF_VERSION format: "146.0.6+g68649e2+chromium-146.0.7680.154"
        ...
        # ----------------------------------------------------------------------
        # libplacebo 7.360.1 (cef-mpv requires >=7.360.1; nixpkgs has 7.351.0)
        # ----------------------------------------------------------------------
        # The cef-mpv branch's meson.build pins a minimum libplacebo version
        # ahead of what nixpkgs currently ships. Bump it just for our mpv —
        # a leaf override, doesn't affect anything else in nixpkgs.
        libplacebo-pinned = pkgs.libplacebo.overrideAttrs (old: rec {
          ...
          src = pkgs.fetchFromGitHub {
            ...
            # Replace with the real hash on first build — `nix build` will
            # fail and print the correct value.
        ...
        # from mpv" and the platform init aborts.
        mpv =
        ...
        # ----------------------------------------------------------------------
        # Common build/runtime dependency lists
        # ----------------------------------------------------------------------
      ...
      in
      {
        ...
        devShells.default = pkgs.mkShell {
          ...
          shellHook = ''
            ...
            Quick build (matches `nix build .`):
              cmake -G Ninja -B build \
                -DEXTERNAL_CEF_DIR=$EXTERNAL_CEF_DIR \
                -DEXTERNAL_MPV_DIR=$EXTERNAL_MPV_DIR
              ninja -C build

            Or use upstream's task runner:
